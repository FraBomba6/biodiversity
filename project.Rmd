...
```{r Libraries, message=FALSE, warning=FALSE, include=FALSE, suppressWarnings = TRUE, suppressPackageStartupMessages = TRUE}
library(knitr)
library(kableExtra)
library(tidyverse)
library(ggraph)
...
I dati sulla biodiversità mondiale sono stati ottenuti dalla rete GBIF attraverso il download dall'apposito portale [@dataset].

Il Global Biodiversity Information Facility è una rete internazionale e un'infrastruttura dati fondata dai governi mondiali che si pone l'obbiettivo di fornire libero accesso ai dati riguardanti tutti i tipi di vita sulla Terra [@gbif.org2022].
...
```{r Read Interactions, include=FALSE}
...
I dati sulla colonia di formiche sono state ottenute da una ricerca [@mersch2013a].
...
```{r View human networks, echo=FALSE, out.height="33%", out.width="33%", fig.align='default'}
paths <- NULL
...
Il boxplot si basa sulla betweenness pesata. Il peso è stato calcolato come $2 - \text{minMaxNorm}(\text{occorrenze})$ , dove le occorrenze sono il numero di interazioni tra due nodi nello stesso giorno.

Possiamo osservere come formiche raccoglitrici abbiano betweenness mediamente più alta rispetto agli altri due gruppi e che anche gli outliers siano nettamente più alti. Questo indica che le formiche raccoglitrici sono centrali all'interno della trasmissione sociale nella colonia.
...
compareNetwork <- function(x){
  g <- as_tbl_graph(colony_interactions %>% filter(day == x, count > 5), directed = FALSE)
